...
export function renderMarkdown(markdown: string, options: RenderMarkdownOptions) {
  return marked.parse(markdown, {
    ...
    gfm: true,
    breaks: true,
    renderer: Object.assign(new marked.Renderer(), options.format === 'gemini' ? gmiRenderer : htmlRenderer),
  });
}
...
export async function renderTemplate(body: string, options: RenderTemplateOptions) {
  const template = await fs.readFile(
    path.join(path.normalize(path.join(options.sourcePath, '../partials')), `/template${options.ext}.hbs`),
  );
